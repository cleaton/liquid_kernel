...
#define gser_debug(xx...) do{} while(0)
/* #define gser_debug(xx...) do{ printk(xx); } while(0) */
...
invalid:
	...
	if (value >= 0) {
		gser_debug("gser ttyGS%d req%02x.%02x v%04x i%04x l%d\n",
...
static int gser_notify_serial_state(struct f_gser *gser)
{
	...
	if (gser->notify_req) {
		gser_debug("gser ttyGS%d serial state %04x\n",
